...
class MBPSubmitTest {
    ...
    void testIocLimitOrderNoFillRejected() {
        BacktestOrder order = makeOrder(100, 10, Side.Bid, "IOC_NO_FILL",
                OrderType.LIMIT, TimeInForce.IMMEDIATE_OR_CANCELED);
        List<BacktestExecutionReport> reports = exchange.submitOrder(order);
    ...
    void testFokLimitOrderNoFillRejected() {
        BacktestOrder order = makeOrder(100, 10, Side.Bid, "FOK_NO_FILL",
                OrderType.LIMIT, TimeInForce.FILL_OR_KILL);
        List<BacktestExecutionReport> reports = exchange.submitOrder(order);
    ...
    void testAutoGeneratedClientOid() {
        BacktestOrder orderWithNull = new BacktestOrder(1, 1, null, Side.Bid, 100, 10,
