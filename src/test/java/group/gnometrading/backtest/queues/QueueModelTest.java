...
class QueueModelTest {
    ...
    static Stream<TestCase> testCases() {
        return Stream.of(
                ...
                new TestCase(
                        ...
                        List.of(
                                new LocalOrderSpec("A", 0, -9, 10),
                                new LocalOrderSpec("B", 0, -10, 10)
                        ),
                        List.of()
                ),
    ...
    void testOnTrade(TestCase tc) {
        ...
        List<LocalOrder> orders = tc.localOrders().stream()
                .map(spec -> {
                    BacktestOrder order = new BacktestOrder(1, 1, spec.clientOid(),
                            Side.Bid, 10000L, 10L, OrderType.LIMIT, TimeInForce.GOOD_TILL_CANCELED);
        ...
        for (int i = 0; i < tc.expectedState().size(); i++) {
            ...
            assertEquals(expected.remaining(), actual.remaining,
                    "remaining for " + actual.order.clientOid());
            assertEquals(expected.phantomVolume(), actual.phantomVolume,
                    "phantomVolume for " + actual.order.clientOid());
            assertEquals(expected.cumulativeTradedQuantity(), actual.cumulativeTradedQuantity,
